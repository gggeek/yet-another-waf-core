...
/// @todo... handle the case of the string ending with '=' without trying to parse '' as StructuredItem
                    ...
                    if ($value[$offset] === ';') {
                        $offset++;
                        $subErrors = [];
                        [$parameters, $newOffset] = self::parseItemParameters(substr($value, $offset), $subErrors);
                        $offset += $newOffset;
                        if ($subErrors) {
                ...
                if ($value[$offset] === ',') {
                    ...
                    $errorsFound[] = 'Invalid Structured Field Dictionary found: invalid char found at end of element value/parameters';
                    $pieces = [];
                }

            } else {
                $errorsFound[] = 'Invalid Structured Field Dictionary found: expected valid element name but did not find it';
                $pieces = [];
                break;
            }
        }
...
/// @todo...
    ...
    {
        [$item, $offset] = self::parseItemInner($value, true, $errorsFound);
        return $item;
    }
     ...
     */
    protected static function parseItemInner(string $value, bool $isItem, array &$errorsFound): array
    {
        $foundType = null;
        $parsedValue = null;
        $parameters = [];
        ...
        if ($len === 1 && in_array($value[$offset], ['-', '"', ':', '?', '@', '%'])) {
            ...
            if ($isItem && !$errorsFound /*&& $offset !== null*/ && $offset < $len && $value[$offset] === ';') {
                $offset++;
                $parametersErrors = [];
                [$parameters, $newOffset] = self::parseItemParameters(substr($value, $offset), $parametersErrors);
                $offset += $newOffset;
                if ($parametersErrors) {
     ...
     *       That should be achievable using regexp_match replacing ^ with \G and passing in an offset
     */
    protected static function parseItemParameters(string $value, array &$errorsFound): array
    {
        $pieces = [];
        $offset = 0;
...
/// @todo... handle the case of the string ending with '=' without trying to parse '' as StructuredParameter
                    [$param, $newOffset] = self::parseItemInner(substr($value, $offset), false, $subErrors);
                    $offset += $newOffset;
